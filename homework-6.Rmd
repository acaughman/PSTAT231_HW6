...
**Answer:** The decision tree preforms better with smaller complexity penalties. It has a peak at mid level complexity.
...
```{r}
tree_metrics = collect_metrics(tree_tune) %>% #get roc auc of all models
  select(mean) %>% #select only mean
  slice_max(n = 1, order_by = mean) %>%  #get top AUC ROC by mean
  mutate(model = "Decision Tree") #add model name
...
**Answer:**
* mtry = the number of predictors randomly sampled when creating each of the trees in the random forest
* trees = the number of individual trees created in the random forest model
* min_n = the number of data points included at each node in the decision tree required for the node to continue to be split
...
**Answer:** A model with mtry = 8 represents a random forest where all the trees are identical because they all use all of the predictors in the model.
...
**Answer:** Minimal node size seems to have little effect on peak accuracy. In general, having a low number of predictors leads to lower accuracy, and this peaks around 3 predictors. It decreases slightly around 6-8 predictors. Number of trees also has minimal effect on accuracy once you have more than 10 trees in your model. 10 trees has lower accuracy than other combinations.
...
```{r}
rf_metrics = collect_metrics(rf_tune) %>% #get roc auc of all models
  select(mean) %>% #select only mean
  slice_max(n = 1, order_by = mean) %>%  #get top AUC ROC by mean
  mutate(model = "Random Forest") #add model name
...
```{r}
rf_fit %>% 
  extract_fit_engine() %>% #extract fit engine
  vip() #visualize the importance of variables
```


Which variables were most useful? Which were least useful? Are these results what you expected, or not?

**Answer:** Special attack is the most important. Speed, attack, hp, special defense, and defense follow with similar levels of importance. Finally, generation and legendary status are the least important variable in the model.
...
```{r}
autoplot(boost_tune) #auotplot tuning results
```

What do you observe? 

**Answer:** Low number of trees are the least accurate. ROC ACU peaks at around 800 trees and then slowly declines again.

What is the `roc_auc` of your best-performing boosted tree model on the folds? 

```{r}
boost_metrics = collect_metrics(boost_tune) %>% #get roc auc of all models
  select(mean) %>% #select only mean
  slice_max(n = 1, order_by = mean) %>%  #get top AUC ROC by mean
  mutate(model = "Boosted Trees") #Add model name
...
```{r}
models = full_join(tree_metrics, rf_metrics)
models = full_join(models, boost_metrics)

models %>% 
  kbl(digits = 2, col.names = c("ROC AUC", "Model")) %>%
  kable_minimal()
```   

**Answer: ** The best model is the random forest model, with an AUC ROC of 0.727.
...
```{r}
conf_mat(rf_test_fit, truth = type_1, estimate = .pred_class) %>% #calclate confusion matrix
...
**Answer:** The model best predicted bug and  normal pokemon. It did a poor job of predicting grass and water pokemon.
...
abalone_recipe = recipe(age ~ type + longest_shell + diameter + height + whole_weight + shucked_weight + viscera_weight + shell_weight, data = abalone_train) %>% #start recpite
...
abalone_rf_tune <- tune_grid( #use cross validation to tune mtry, trees, and min_n parameters
  ...
  metrics = metric_set(rmse) #tune based on ROC AUC
  )
```
...
abalone_rf_fit = fit(abalone_rf_final, abalone_train) #fit final rf model to training data
```

```{r}
augment(abalone_rf_fit, new_data = abalone_test) %>% #predict on test data
  rmse(truth = age, estimate = .pred) #get rmse for testing set
```

**Answer:** The RMSE for the testing data set is 2.10
